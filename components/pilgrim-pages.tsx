    ...
    <>
      ...
      <div className="pilgrim-home-grid">
        ...
        <section className="quick-actions card">
          ...
          <div className="quick-grid"><Link href="/app/assistant"><span className="quick-icon green"><Bot/></span><b>{tr("اسأل رفيق", "Ask Rafeeq")}</b><small>{tr("إجابة مرتبطة برحلتك", "Journey-aware help")}</small><ChevronLeft/></Link><Link href="/app/location"><span className="quick-icon gold"><LocateFixed/></span><b>{tr("موقعي", "My location")}</b><small>{tr("الاتجاه والازدحام", "Directions and crowds")}</small><ChevronLeft/></Link><Link href="/app/bracelet"><span className="quick-icon mint"><Watch/></span><b>{tr("السوار", "Bracelet")}</b><small>{tr("البطارية والحساسات", "Battery and sensors")}</small><ChevronLeft/></Link><button onClick={() => setSosOpen(true)} className="sos-quick"><span className="quick-icon red"><Siren/></span><b>{tr("طلب مساعدة", "Request help")}</b><small>{tr("شارك موقعك وبياناتك", "Share location and vitals")}</small><ChevronLeft/></button></div>
        </section>
...
export function AssistantPage() {
  ...
  return <div className="assistant-layout"><section className="chat-panel card"><header className="chat-header"><span className="bot-avatar"><Bot/></span><div><h1>اسأل رفيق</h1><span><i/>مساعدك السياقي · متصل</span></div><button onClick={() => notify("تم تفعيل القراءة الصوتية للردود") }><Volume2/></button></header><div className="context-strip"><span><MapPin/><small>موقعك</small><b>عرفة B-12</b></span><span><MoonStar/><small>المرحلة</small><b>الوقوف بعرفة</b></span><span><HeartPulse/><small>الصحة</small><b>مستقرة</b></span></div><div className="messages" aria-live="polite"><div className="conversation-date">اليوم</div>{messages.map((message, index) => <div key={index} className={`message ${message.from}`}><span>{message.from === "bot" ? <Bot/> : "ع"}</span><div><p>{message.text}</p><time>{message.time}</time>{message.from === "bot" && /مختص/.test(message.text) && <button className="authority-button" onClick={() => notify("هذه قناة تجريبية: سيتم ربطك بالجهة الدينية المعتمدة") }><PhoneCall/>تواصل مع جهة معتمدة · تجريبي</button>}</div></div>)}</div><div className="suggested-prompts">{suggestedPrompts.map((prompt) => <button key={prompt} onClick={() => answer(prompt)}>{prompt}</button>)}</div><form className="chat-input" onSubmit={submit}><button type="button" className={listening ? "listening" : ""} onClick={() => { setListening(!listening); notify(!listening ? "جارٍ الاستماع - محاكاة" : "تم إيقاف الاستماع"); }}><Mic/></button><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="اكتب سؤالك هنا..." aria-label="رسالتك إلى رفيق"/><button type="submit" disabled={!input.trim()}><Send/></button></form><footer className="chat-disclaimer"><ShieldCheck/>قد يقدم رفيق معلومات إرشادية، لكنه لا يشخّص الحالات الطبية ولا يصدر فتاوى.</footer></section><aside className="assistant-aside"><section className="assistant-context card"><span className="eyebrow">يعرف سياقك</span><h2>إجابة مناسبة لرحلتك</h2><ul><li><CheckCircle2/>موقعك ومرحلتك الحالية</li><li><CheckCircle2/>موعد وجهتك التالية</li><li><CheckCircle2/>ملخص حالتك الصحية</li><li><CheckCircle2/>تنبيهات مجموعتك</li></ul></section><section className="religious-card card"><div className="card-label"><span><MoonStar/>إرشاد ديني موثوق</span><ShieldCheck/></div>{trustedReligiousCards.map((card) => <button key={card.title} onClick={() => answer(card.title)}><div><b>{card.title}</b><small>{card.detail}</small></div><ChevronLeft/></button>)}</section></aside></div>;
}

export function ProfilePage() {
  const { notify } = useRafeeq();
  const [emergencyView, setEmergencyView] = useState(false);
  return <><section className="page-heading"><div><span className="eyebrow">الملف الطبي</span><h1>معلوماتي الصحية والطوارئ</h1><p>معلومات تساعد الطاقم الطبي المصرح له على الاستجابة بسرعة.</p></div><button className="outline-button" onClick={() => { setEmergencyView(!emergencyView); notify(!emergencyView ? "تم فتح معاينة الطوارئ" : "تم إغلاق معاينة الطوارئ"); }}><Stethoscope/>{emergencyView ? "عرض الحاج" : "معاينة الطوارئ"}</button></section>{emergencyView && <section className="emergency-access-banner"><ShieldCheck/><div><b>عرض الطوارئ المصرح</b><p>يعرض الحد الأدنى الضروري من المعلومات للطواقم الطبية.</p></div><span>DEMO</span></section>}<div className="profile-layout"><section className="identity-card card"><div className="identity-avatar">ع ح</div><h2>{pilgrim.name}</h2><p>{pilgrim.id}</p><span><MapPin/>{pilgrim.currentLocation}</span><div className="identity-grid"><span><small>العمر</small><b>{pilgrim.age} سنة</b></span><span><small>فصيلة الدم</small><b className="blood-type">{pilgrim.bloodType}</b></span><span><small>الجنسية</small><b>{pilgrim.nationality}</b></span><span><small>المجموعة</small><b>{pilgrim.group}</b></span></div></section><div className="medical-info-grid"><InfoCard icon={HeartPulse} title="الحالات المزمنة" items={pilgrim.chronicConditions}/><InfoCard icon={AlertTriangle} title="الحساسية" items={pilgrim.allergies} alert/><InfoCard icon={Pill} title="الأدوية الحالية" items={pilgrim.medications}/><section className="info-card card emergency-contact"><div className="info-card-head"><span><PhoneCall/></span><h3>جهة اتصال الطوارئ</h3></div><b>{pilgrim.emergencyContact}</b><button onClick={() => notify("تم بدء اتصال تجريبي بجهة الطوارئ") }><PhoneCall/>اتصال تجريبي</button></section></div></div><section className="privacy-card card"><LockKeyhole/><div><b>من يمكنه رؤية هذه البيانات؟</b><p>يمكنك رؤية ملفك كاملًا. لا يصل الطاقم الطبي إلا للمعلومات الضرورية عند وجود حالة مصرح بها، بينما ترى غرفة العمليات بيانات مجمعة مجهولة الهوية فقط.</p></div><button onClick={() => notify("سجل الوصول التجريبي: لم تُفتح البيانات إلا بواسطتك")}>سجل الوصول</button></section></>;
}
...
export function BraceletPage() {
  ...
  return <><section className="page-heading"><div><span className="eyebrow">السوار الذكي</span><h1>حالة الاتصال والحساسات</h1><p>السوار يجمع القراءات ويرسلها إلى هاتفك عبر Bluetooth.</p></div><StatusDot>{bracelet.connected ? "متصل الآن" : "غير متصل"}</StatusDot></section><div className="bracelet-layout"><section className="bracelet-hero card"><div className="bracelet-visual"><div className="strap top"/><div className="watch-face"><HeartPulse/><span className={`led ${led}`}/><b>78</b><small>BPM</small></div><div className="strap bottom"/></div><div className="bracelet-summary"><span className="eyebrow">سوار عبدالله</span><h2>Rafeeq Band 01</h2><p><Bluetooth/>متصل بهاتفك الآن</p><div className="battery-row"><span><BatteryCharging/><b>{bracelet.battery}%</b></span><div><i style={{ width: `${bracelet.battery}%` }}/></div></div><small>آخر مزامنة: {bracelet.lastSync}</small><button className="primary-button" onClick={sync} disabled={syncing}><RefreshCw className={syncing ? "spin" : ""}/>{syncing ? "جارٍ المزامنة..." : "مزامنة الآن"}</button></div></section><section className="connection-card card"><div className="card-label"><span><Radio/>حالة الاتصال</span><RiskBadge level="normal" label="ممتاز"/></div><div className="connection-list"><span><Bluetooth/><div><b>Bluetooth</b><small>متصل · إشارة قوية</small></div><CheckCircle2/></span><span><Nfc/><div><b>NFC</b><small>{bracelet.nfc}</small></div><CheckCircle2/></span><span><SmartphoneIcon/><div><b>GPS الموقع</b><small>من هاتف الحاج، وليس من السوار</small></div><Info/></span></div></section></div><SectionTitle eyebrow="الأجهزة" title="حالة الحساسات"/><div className="sensor-grid">{bracelet.sensors.map((sensor) => <article className="card" key={sensor.name}><span><Activity/></span><div><b>{sensor.name}</b><small>الفحص التلقائي مكتمل</small></div><StatusDot>{sensor.state}</StatusDot></article>)}</div><section className="led-section card"><div><span className="eyebrow">مؤشر السوار</span><h2>دليل ألوان LED</h2><p>إشارة مبسطة تساعد الحاج على فهم مستوى التنبيه، ولا تمثل تشخيصًا طبيًا.</p></div><div className="led-options"><button className={led === "white" ? "active" : ""} onClick={() => setLed("white")}><i className="white"/><b>أبيض</b><small>الحالة مستقرة</small></button><button className={led === "orange" ? "active" : ""} onClick={() => setLed("orange")}><i className="orange"/><b>برتقالي</b><small>راحة وماء ومراقبة</small></button><button className={led === "red" ? "active" : ""} onClick={() => setLed("red")}><i className="red"/><b>أحمر</b><small>توقف واطلب المساعدة</small></button></div><button className="outline-button" onClick={() => notify(`تم إرسال اهتزاز تجريبي مع مؤشر ${led === "white" ? "أبيض" : led === "orange" ? "برتقالي" : "أحمر"}`)}><Radio/>اختبار الاهتزاز والمؤشر</button></section></>;
}
